...
Visually, the first thing that jumps out to me that there seem to be a few outliers that may be influencing the results. Let's call a group "fast" if that group has more than `r value_threshold` seconds left at the end of the session. The teams that are labeled had more than `r value_threshold` seconds (`r value_threshold / 60` minutes) remaining at the end of the session.  The groups that are labeled in all the conditions (Target levels) are team 26, 45, 13. Some of these teams data may be outliers. 

*Reflection:* This figure tells me that I may need to consider removing these teams because they may be outliers. However, I do not have a good reason to remove the values. 
...
In the individual condition, there is a steadying increase in the time remaining at the individual level. 
...
*Reflection:* There is a session that something interesting is going on with the Ind_Team and team conditions. It seems as though the effect of the team condition happen quicker than the impact of the Ind_Team condition

#### Summary / Reflection

I am getting the sense that there is something interesting happening with the Ind_Team condition and team condition. For some reason, the positive effect (i.e., the large increase in the time remaining at the end of the session) happens quicker than the positive effect from the Ind_Team condition. More exploration is needed to understand why this apparent effect is happening. Some statistical analysis is necessary to determine the strength of this effect if it exists.
...
A note on the effect size ($R^2$). This method generates two types of ($R^2$) values called marginal ($R^2$) ($R_{m}^2$) and conditional ($R^2$) ($R_{c}^2$). The $R_{m}^2$ describes how the variance is described by the fixed effect variables (feedback condition and session order) while the $R_{c}^2$ describes how both the fixed and random effect variables describe the variance

#### Full Models Used for Team and Individual

The next two sections below describe the full model for team and individual level analysis. Eight models were fitted and then compared. Eight other models are a subset of the full model (including a null model that has no fixed effect variables). I removed one or two fixed effects from the full model. 
...
comparision_results <- anova(model.null, 
                             model.All, 
                             model.NoInteraction,
...
# Effect size (R^2)
ES_model.NoInteraction.NoTarget <- r.squaredGLMM(model.NoInteraction.NoTarget)

ES_model.NoInteraction.NoTarget %>%
  kable(caption = "Table: Effect size for *No Interaction and No Target*") %>%
   kable_styling(full_width = F)

ES_model.NoInteraction <- r.squaredGLMM(model.NoInteraction)

ES_model.NoInteraction %>%
...
### Q: Are any of the effects in the two models (model.NoInteraction.NoTarget and model.NoInteraction) statistically different from zero?

The purpose of this question is to see what we can learn from these models. Before I examine the model, I will test the assumption about the residuals of the models. The assumptions are as follows [@Galwey2014]:

* The residuals should have an approximately normal distribution (i.e., a bell curve) when plotted on a histogram
* A fitted-value plot should show an almost constant width when viewed from left to right
* The points on a normal plot should lie on an almost straight line from the bottom left to the top right

Some of the assumptions may be violated. According to @Field2017, the best way to examine the influence of assumption violations is to compare a robust model to a standard model. When the assumptions are violated, I will compare a robust model to a classic model. If there is a noticeable different between the two models (i.e., if the estimated coefficient are noticeably different) then I will use a step on the power ladder to transform, or re-express, the data to address the assumption violation @Tukey1977.
...
Visually, it looks like assumptions are violated in the QQ plot and in the fitted-value plot. This means that I will need to transform the data.
...
*Reflection:* It looks like there may be some assumption violation for both models. 

For the model.NoInteraction it looks like there may be a violation of constant error in the fitted-value plot. The QQ plot also shows a slight violation on the tail ends of the plots. 

For the model.NoInteraction.NoTarget it looks like there may be a violation of constant error in the fitted-value plot. The QQ plot also shows a slight violation on the tail ends of the plots. 

I need to determine if this violation influences the model overall.
...
The robust model.NoInteraction model is noticeably different when compared to the non-robust model.NoInteraction. This suggests that the data should be transformed to satisfy the residual assumptions
...
The robust model.NoInteraction.NoTarget model is noticeably different when compared to the non-robust model.NoInteraction.NoTarget This suggests that the data should be transformed to satisfy the residual assumptions
...
# Histogram of Residuals - model.NoInteraction
residuals_formula_NoInteraction <- residuals(model.NoInteraction)
ggplot(model.NoInteraction, aes(x = .resid)) +
  geom_histogram(bins = 30) + 
  labs(title = paste("Histogram of Residuals for model.NoInteraction"), x = "", y = "") +
  theme(plot.title = element_text(hjust = 0.5))
...
*Reflection:* The figures suggest that the data transformation resolved the residual violations for both models. I will use the square root transformation and model that data transformation.

##### Q: Are there any effects that are significant?

I want to determine if there is a significant effect from the Target variable. I will examine the model itself, and then I will calculate the estimated marginal means to compare all three levels.
...
The estimations are in reference to the individual level for the Target variable and session 2 for the session order variable. The results suggest that session 3 and 4 have a significant effect. This is expected. The results also indicate that the impact of the team and Ind_Team target levels do not have an effect that is greater than zero.
...
```
The estimated marginal means show no significant difference among the Target variable. 
...
The estimated marginal means show a significant difference between all of the sessions. This suggests that the amount of time remaining at the end of the session increased over time. 

*Reflection:* The *model.NoInteraction:* model shows no significant effect from the Target levels, but it does show a significant effect from the session order. Specifically, all of the session order were significantly different from one another. This means that the amount of time remaining at the end of each session increased over time. 
...
I had to transform the data to resolve some of the assumptions issues. I used one of the steps in the Tukey ladder @Tukey1977. I took the square root of all the values in the time remaining value for the teams.

Overall, the results show no significant effect from the Target variable, but it does show that the session order has a significant impact. This means that the amount of time remaining at the end of each session increased over time. This is expected. There are no significant results that can be discussed about the Target variable because there were no statistically significant results.
...
Visually, it looks like there may be a violation of constant error in the fitted value plot. It seems like there may also be a violation in the QQ plot (on the tail end of the plot). Will need to fit the robust model to see if I need to transform the data to resolve some of the assumption violations.
...
It looks like there might be a violation with the constant error assumption (based on the fitted value plot) and there looks like there may be a violation with normality based on the QQ plot. I will need to fit a robust model to see if these apparent violations influence the model fit.

*Reflection:* Overall, it looks like I need to compare the models to the robust models to see if I need to worry about the assumption violations
...
There is a noticeable difference between the non-robust model and the robust model. This indicates that the data should be transformed to attempt to satisfy the assumptions.
...
*Reflection:* Overall, I need to transform the data because there is a very noticeable difference between the robust model and the non-robust model.
...
Comparing the robust and non-robust model does not reveal any noticeably different value.
...
# Generate models with variable response
model.NoInteraction.NoTarget <- model_data_Target_Session(df = data_focus_ind, dependent =  response_variable, model.type =  "NoInteraction_NoTarget", is.team = FALSE, is.robust = FALSE)
rmodel.NoInteraction.NoTarget <- model_data_Target_Session(df = data_focus_ind, dependent =  response_variable, model.type =  "NoInteraction_NoTarget", is.team = FALSE, is.robust = TRUE)

compare(model.NoInteraction.NoTarget, rmodel.NoInteraction.NoTarget, show.rho.functions = FALSE)
...
With reference to session 2, there is a significant effect from session 3 and 4. This suggests that the amount of time (square root) remaining increase when going from session 2 to session 3 and from session 2 to session 4. 
