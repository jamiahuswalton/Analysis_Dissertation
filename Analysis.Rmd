...
I want to examine the distribution of collection rates. 
...
Where $duration_{Individual}$ is the amount of time the indvidiaul spent completing the task and $total.correct.items.collected_{Individual}$ is the total number correct items (individual or team items) collected by the individual. 

I divided the duration by the number of items to collect because dividing the total correct items collected by the duration has hard to interpret. 
...
ggplot(data = plot_data_team) + 
  geom_histogram(aes_string(x = dependent_response_team), bins = 30) +
  labs(title = title_response_team, x = x_label_team, y = y_label_team) +
  ggsave("Histogram_CollectionRateCorrect_Team.png")
```
...
The distrabution plot is similar to the the overall distrabution plot. However, the majority of participants in the Team condition seem to need around 50 seconds to collect a correct item while participants in the Ind condition needed 60 or 70 seconds.

#### Summary / Reflection

At the team level, most teams needed about 20 seconds on average to collect a correct team item. The center of the distrabutions when seperated by Target are roughly similar. The centers were also hovering around 20 for each distribution.

At the individual level, most participants needed about 50 seconds to collect a correct item. When I group the data by Target levels, the distribution is similar to the overall distrubution. 

How does the session order influence the collection rate distibution?

The next step is to see how the distrabutions look when the data is group by Target and Session Order. 

## The section below is for the collection rate for correct items. Still nees work XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
### Q: Is there exciting variation among the *Collection_rate_correct_item* that explains how the feedback conditions influence the time remaining when session order is taken into consideration?


```{r}
# Data to model
data_modified_ind <- ind_data %>%
  select(Collection_rate_correct_item_ind,
         Target, 
         SessionOrder, 
         Team, 
         Player_ID, 
