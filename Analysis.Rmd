...
The skewed to the left pattern continues when grouping the time remaining by the feedback conditions. Most of the teams seem to have about 50 seconds remaining when they completed the tast. 

*Reflection:* The skewness in the time remaining data at the team level(Something missing here). This skewness suggests that most of the teams had about the same amount of time remaining at the end of the session (50 or 40 seconds). 
...
*Reflection:* The distribution is similar to the distribution of the time remaining at the team level. The distribution skewed to the left and most of the time remaining at the end of the session was about 50 or 60 seconds  
...
It looks like the time remaining for the individual_team condition seems to spread over time, which means that the teams were completing the task faster the more times they completed the task. I would not be surprised if the Individual_Team condition were trending to significant.  
  
  
It looks like the distribution of the amount of time remaining at the end of the session in the team feedback condition starts to spread over time. This distribution is what I would expect to see.
...
Some of the assumptions may be violated. According to @Field2017, the best way to examine the influence of assumption violations is to compare a robust model to a standard model. When the assumptions are violatd, I will compare a robust model to a classic model. If there is a noticable different between the two models (i.e., if the estimated coefficence are notciably different) then I will use a step on the power ladder to trasform, or re-express, the data to address the assumption violation @Tukey1977.
...
Visually, it looks like assumptions are violated in the QQ plot and in the fitted-value plot. This means that I will need to trasnform the data.
...
The robust model.NoInteration model is noticeably different when compared to the non-robust model.NoInteration. This suggests that the data should be transformed to satisfy the residual assumptions
...
comparision_results %>%
  ...
  kable_styling()
```
There is a noticable difference between the non-robust model and the robust model. This indicates that the data should be transformed to attempt to satisfy the assumptions.
...
With reference to session 2, there is a significant effect from session 3 and 4. This suggests that the amount of time (sqaure root) remaining increase when going from session 2 to session 3 and from session 2 to session 4. 
