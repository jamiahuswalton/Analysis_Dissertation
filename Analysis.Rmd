...
My next step is to examin the models and see what I can learn from these models. Are any of the effects in these models significantly different from zero?
...
The purouse of this questions is to see what we can learn from these models. Before I examine the model I will test the assumption about the residuals of the models. The assumtions are as follows [@Galwey2014]:
...
It looks like there are some assumption violation for both models. 

For the model.NoInteraction it looks like there may be a violation of constran error in the fitted-value plot. The QQ plot also shows a slight violation on the tail ends of the plots. 

For the model.NoInteraction.NoTarget it looks like there may be a violation of constran error in the fitted-value plot. The QQ plot also shows a slight violation on the tail ends of the plots. 

I need to determine if this violation influences the model over all.

##### Q: Does the violation influence the linear model estimation?

I will used the robust estimation for linear mixed models found in the robustlmm package by @Koller2016. I will then compare the robust model to the non-robust model. If the estimated values are noticablly different then data will need to be transformed. 
...
The robust model.NoInteration model is noticablly different when compared to the non-robust model.NoInteration. This suggest that the data should be transformed to satisfy the residual assumptions
...
The robust model.NoInteraction.NoTarget model is noticablly different when compared to the non-robust model.NoInteraction.NoTarget This suggest that the data should be transformed to satisfy the residual assumptions

*Reflection:* The comparing the normal estimation to the robust models indicates that the data should transformed becuase the estimations are noticablly different.

##### Q: Does data transformtion satisfy the assumptions of the residuals?
...
*Reflection:* The figures suggest that the data transformation resolved the resdiual violations for the both models. I will use the square root transformation and model that data transformation.
...
The estaimations are in reference to the individual level for the Target variable and session 2 for the session order variable. The results suggest that that session 3 and 4 have a significant effect. This is expected. The results also suggest that the effect of the team and Ind_Team target levels do not have an effect that is greater than zero.
...
The estimated marginal means show a significant different between all of the sessions. This suggests that the amount of time remaining at the end of the session increased over time. 
...
Overall, the results show no significant effect from the Target variable but it does show that session order had a significant effect. This means that the amount of time remaining at the end of each session increased over time. This is expected. There are no significant results that can be discussed about the Target variable because there were no statiscally significant results.
...
Visually, it looks like there may be a violation of constant error in the fitted value plot. It looks like there may also be a violation in the QQ plot (on the tail end of the plot). Will need to fit the robust model to see if I need to transform the data to resolve some of the assumption violations.
...
It looks like there migt be a violation with the constant error assumption (based on the fitted value plot) and there looks like there may be a violation with normallity based on the QQ plot. I will need to fit a robust model to see if these apprent violations influence the model fit.

*Reflection:* Overall, it looks like I need to compare the models to the robust models to see if I need to worry about the assunption violations
...
*Reflection:* Overall, I need to transform the data because there is a very noticable difference between the robust model and the non-robust model.
...
Comparing the robust and non-robust model does not reveal any noticably different value.
...
The results showed a significant effect from session 3 and 4 when compared to session 2. The results showed no signifcant effect from Target levels. However, it is important to note that the effect from the Team Target level is approaching significance (p = .0779). This suggest that more data is needed to reach significance. 
...
emmeans(model.NoInteraction, list(pairwise ~ Target), adjust = "tukey")
```
The results show a significant difference between the Int_Team and Team level of feedback. The difference indicates that amount of time remaining in the Team feedback condition results in higher time remaining. 
