...
model.All <- model_data_Target_Session(df = GA_data_focus_ind, dependent =  params$dependent_response_ind, model.type =  "All", is.team = FALSE, is.robust = FALSE)
rmodel.All <- model_data_Target_Session(df = GA_data_focus_ind, dependent =  params$dependent_response_ind, model.type =  "All", is.team = FALSE, is.robust = TRUE)
...
Overall, none of the violations seem to have a noticable influence on the model when comepared to the robust model. I will not transform the data. 
...
__Note:__ It is important to note that the model __failed to converge__. This means that the model was not able to be fitted. Any analysis from this model should not carry a lot of weight. 

The results show a __significant effect__ from Ind_Team in reference to Ind and the results show a __margnially significant__ effect from the Team conditio in reference to Ind condition. There is a significant effect from session 3 in reference to session 2 but there is no marginally significant effect from session 4 in reference to session 2. 

Inturpriting the main effects is made difficult when including the interaction effect.
...
The result show no significant difference among the Target levels.
...
The is a significant difference between session 2 and 4, indicating that frustration is higher in session 4. 
...
The result show no significant differences. 

__Note:__ Model failed to converge. 
...
```
The restuls show a significant effect from session 4 in reference to session 2.
...
Results showed no significant difference among the session orders. The results showed a __margnially significant__ difference between session 2 and 4.
...
The results show a significant effect from Team and Ind_Team in reference to Ind condition. The results also show a significant effect from from session 4. 
...
The results show a __significant difference__ between Ind and Ind_Team, and between Ind and Team. 
...
The results show a significant difference between session 2 and 4. 

## Summary

Overall, at the individual level the results show a significant difference in self reported frustration when teams used Go Together. There was a __significant difference__ between Ind and Ind_Team, and between Ind and Team. Specifically, the results suggested that when teams used Go Together, frustration was significantly reduced when teams were in the Team or Ind_Team condition, when compared to the Ind condition. There was a bigger reduction in frustration in the Team condition. 

The results suggest that the Team and Ind_Team condition condition maay have reduced frustration.
