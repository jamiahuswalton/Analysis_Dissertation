---
title: 'Dissertation Analysis: Team Orientation Analysis - Correlations'
...
The motivation behind this analysis is to discover different correlations between participants actual performance and self-reported performance. Significance was found in previous analysis and the goal of this analysis is to attempt to understand why their was significance. 
...
# Is there correlation between a actual performance (e.g., time remaining, errors, etc.) and self-reported performance (e.g., My individual performance: "Average")? Is there anything interesting when grouping by Target?
...
ggplot(data = plot_data_ind, aes(x = performance_reverse, y = .data[[dependent]])) +
  geom_point() +
  facet_grid( . ~ Target)
```
...
No obvious patterns. The varience seems to increase over time. 
...
No obious pattern. The varience may increase over time. 
...
```
Overally, it looks like most participants rated their performance as Good or average. Few pariticpants said their performance was Very poor. There were a decent number of participants that said their performance was Average or Excellent. 
...
When grouped by Target, the most obvious correlations appear when participants rated their performance as excellent. There may be some correlations with participants who rated their performance as good. 
...
No obvious pattern, there maybe a correlation for individuals who rated their performance as excellent. 

```{r}
dependent<- "timeRemaining_ind"
y_lab_indep <- "Time Remaining"
...
### Any statisitical significane in correlation?
...
for (my_target in c("Ind", "Team", "Ind_Team")) {
  ...
  print(cor.test(x = plot_data_ind[[dependent]], y = plot_data_ind[["performance_reverse"]], method = "spearman"))
  }
```

Grouping by Target across Session order, there was a __significant__ correlation for Ind and Team condition. Specifically, there was negative correlation between time remaining 
...
# cor.test(x = plot_data_ind[[dependent]], y = plot_data_ind[["performance_reverse"]], method = "spearman")
...
for (ind_rating in c("Excellent", "Good", "Average", "Poor", "Very poor")) {
  for (my_target in c("Ind", "Team", "Ind_Team")) {
    for (my_session in c(2:4)) {
      ...
      if(length(plot_data_ind[[dependent]]) < 3){
        ...
        cor_test_results<- cor.test(x = rank(plot_data_ind[["performance_reverse"]]), y = rank(plot_data_ind[[dependent]]), method = "spearman")
...
N_paris<- length(plot_data_ind[[dependent]])
