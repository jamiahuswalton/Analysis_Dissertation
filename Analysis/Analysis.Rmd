...
In the Ind condition there seems to be little change in distribution of the data. 

In the Ind_Team condition, session 2 and 3 seems to have similar distibutions. Session 4 distribution seems to move more to right. 

In the Team condition, distribution in session 2 and 3 are similar but session 4 has more of skew to the right. 

In session 4, it seems as though the Ind and Ind_Team condition have similar distribution and the distibution in the Team condition. 

_Reflection:_ Overall, there appears to be no obvious patter. It is worth noting that the distribution in the Ind conditions seems to go unchanged from session to session. This could mean that the invention had no effect of had a negative impact on participants collecting correct individuals items. In the Ind_Team and Teamcondition, it looks like a major improvement (i.e., a more obvious skew to the right) happens when moving from session 3 to session 4. This indicates that something happens between sessions 3 and 4 to increase performance. It seems as though the skew in the Team condition in session 4 is greater than the skew in the Ind and Ind_Team conditon.
...
value_threshold <- 120
plot_name <- "Bar_CI_ByTarget_BySessionOrder_Team.png"
setwd(figure_directory)
